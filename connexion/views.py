...
def profile(request):
    if request.method == 'POST':
        ...
        if unit == 'C':
            api_url = urllib.request.urlopen(
                'http://api.openweathermap.org/data/2.5/weather?q=' + city + '&units=metric&appid=252ed3179ad46545d339cf4bbcc3723b').read()
            api_url2 = json.loads(api_url)
        elif unit == 'F':
            api_url = urllib.request.urlopen(
                'http://api.openweathermap.org/data/2.5/weather?q=' + city + '&units=imperials&appid=252ed3179ad46545d339cf4bbcc3723b').read()
            api_url2 = json.loads(api_url)
        else:
            api_url = urllib.request.urlopen(
                'http://api.openweathermap.org/data/2.5/weather?q=' + city + '&units=metric&appid=252ed3179ad46545d339cf4bbcc3723b').read()
        ...
        api_url5 = urllib.request.urlopen(
            'http://api.openweathermap.org/data/2.5/air_pollution?lat=' + str(lat) + "&lon=" + str(
                lon) + "&appid=252ed3179ad46545d339cf4bbcc3723b").read()
    ...
    return render(request, 'connexion/logged.html', {"city": city, "data": data})